...
export class BotCordClient {
  ...
  private observabilityHeaders(requestId = randomBytes(16).toString("hex")): Record<string, string> {
    return {
      "X-BotCord-Request-ID": requestId,
      "X-BotCord-Caller": CLIENT_NAME,
      "X-BotCord-Caller-Version": CLIENT_VERSION,
      "X-BotCord-Agent-ID": this.agentId,
      // A key id is a public credential reference, not private key material.
      "X-BotCord-Credential-Key-ID": this.keyId,
    };
  }
  ...
  async ensureToken(requestId?: string): Promise<string> {
    if (
    ...
    return this.refreshToken(undefined, "missing_or_expiring", requestId);
  }
  ...
  private async performTokenRefresh(
    ...
    try {
      resp = await fetch(`${this.hubUrl}/registry/agents/${this.agentId}/token/refresh`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...this.observabilityHeaders(requestId) },
  ...
  private async hubFetch(path: string, init: RequestInit = {}): Promise<Response> {
    const observabilityHeaders = this.observabilityHeaders();
    const requestId = observabilityHeaders["X-BotCord-Request-ID"];
    let token = await this.ensureToken(requestId);

    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      const headers: Record<string, string> = {
        ...((init.headers as Record<string, string>) ?? {}),
        Authorization: `Bearer ${token}`,
        // These are generated by this client and cannot be overridden by a
        // per-call header bag.
        ...observabilityHeaders,
        "X-BotCord-Retry-Attempt": String(attempt),
      };
      ...
      if (resp.status === 401 && attempt === 0) {
        token = await this.refreshToken(token, "rest_401", requestId);
        continue;
      }
  ...
  async uploadFile(
    ...
    const resp = await fetch(`${this.hubUrl}/hub/upload`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, ...this.observabilityHeaders() },
      body: formData,
      signal: AbortSignal.timeout(30000),
    });
