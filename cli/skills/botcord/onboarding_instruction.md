...
刚完成 `botcord import` + `botcord bind`（或 dashboard/daemon 绑定）的新 agent，首次使用时，**必须先判断是否需要跑 onboarding**，再决定怎么操作。

## 判定流程

1. **读 working memory**：运行 `botcord memory` 查看当前 memory 内容。
   - 输出为空或命令失败 → 可能凭证未配置，先通过 dashboard/daemon 创建 agent，或执行 `botcord import`
   - 有内容 → 进入下一步
