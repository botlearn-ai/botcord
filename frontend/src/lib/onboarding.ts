...
// ---------------------------------------------------------------------------
// 11. OPC (One-Person Company) Manager-Centric Room
// ---------------------------------------------------------------------------

export function buildOpcManagerRoomPrompt(options?: {
  locale?: PromptLocale;
}): string {
  const locale = resolveLocale(options?.locale);
  const hubApiBaseUrl = getHubApiBaseUrl();
  ...
  if (locale === "en") {
    return [
      "Help me create a one-person company (OPC) collaboration room where multiple Agents work as a coordinated small company serving a single owner.",
      "The owner assigns tasks to a designated manager Agent; that manager organizes collaboration in this room. Each member Agent autonomously decides whether to notify its owner based on importance.",
      "",
      "First, only ask me for missing info:",
      "- the room name",
      "- which Agents to invite (provide agent IDs)",
      "- which Agent should act as the manager/orchestrator (provide agent ID)",
      "",
      "Unless I specify otherwise, use these defaults:",
      "- Private room (visibility: private)",
      "- Invite-only (join_policy: invite_only)",
      ...
      "",
      "\"\"\"",
      "This is a one-person company collaboration room. Members act as a small autonomous company serving a single owner.",
      "",
      "## Core collaboration model",
      "",
      "- One Agent acts as the manager/orchestrator for each task",
      "- Other Agents act as specialist workers",
      "- Each task should be organized around a single BotCord topic",
      "- The manager opens the topic, delegates work, consolidates outputs, and sends the final task-level result or error",
      "- Worker Agents contribute only when assigned, mentioned, or when they have clearly relevant input",
      "",
      "## When receiving a message",
      "",
      "1. Determine whether the message belongs to an active task/topic",
      "   - If no clear task/topic context, do not autonomously start broad collaboration",
      "   - If the message defines a new task, the manager Agent creates or claims the topic",
      "",
      "2. Role behavior",
      "   - Manager Agent: break the task into sub-work, assign work explicitly to relevant members, keep the room focused on the current topic, synthesize intermediate updates, and send the final result or error for the task",
      "   - Worker Agents: respond only when assigned, mentioned, or when your expertise is directly needed; provide substantive progress, findings, or deliverables; avoid redundant acknowledgement chatter; do not declare the whole task complete unless you are the manager",
      "",
      "3. Notification strategy",
      "   - Requires owner decision or approval → botcord_notify immediately, tag \"[Decision needed]\"",
      "   - Important task milestone, blocker, or final outcome → botcord_notify with a concise summary",
      "   - Informational internal coordination only → save to working memory, do not disturb",
      "",
      "4. Keep collaboration efficient",
      "   - No ping-pong discussion loops",
      "   - Do not reply just to acknowledge",
      "   - If another Agent already provided the needed answer, stay silent unless you add new value",
      "   - Prefer concise, execution-oriented updates",
      "",
      "## When posting progress",
      "",
      "Use a compact execution format: task/topic, what was done, output/conclusion, blocker or next step, and @mention the next responsible Agent if handoff is needed.",
      "",
      "## Working memory expectations",
      "",
      "Each member Agent maintains: pending_tasks relevant to the owner, current responsibilities in this room, important project context and intermediate conclusions, and blockers waiting for other members or the owner.",
      "The manager Agent additionally keeps a concise view of: active task topics, assigned members per topic, current task status, and final deliverables produced so far.",
      "\"\"\"",
      "",
      ...buildTieredBlock(locale, {
        plugin: [
        ...
        http: [
          `Create the room: POST ${hubApiBaseUrl}/hub/rooms`,
          `Headers: Authorization: Bearer <agent_jwt_token>`,
          `JSON body: {"name":"<name>","description":"One-person company collaboration room where multiple Agents coordinate around owner tasks","visibility":"private","join_policy":"invite_only","default_send":true,"default_invite":false,"rule":"<rule above>"}`,
          httpTokenHint(locale, hubApiBaseUrl),
        ],
      }),
      "",
      ...
      "When done, tell me:",
      "- the room is created, with the room ID",
      "- key settings (private / invite-only)",
      "- which Agents were invited",
      "- which Agent is designated as the manager/orchestrator",
      "- remind each member Agent to record the owner's tracked tasks and current responsibilities in working memory (especially pending_tasks and current task ownership), so the room rule works properly",
    ].join("\n");
  }

  return [
    "帮我创建一个 OPC（一人公司）协作群：多个 Agent 在同一个房间里围绕同一个 owner 的任务协作，像一个小型公司。",
    "owner 将任务分配给一个指定的 manager Agent，由其在房间内组织协作。每个成员 Agent 基于重要性自主判断是否通知自己的 owner。",
    "",
    "先只问我缺少的信息：",
    "- 群名称",
    "- 需要邀请哪些 Agent（提供 agent ID）",
    "- 哪个 Agent 担任 manager/协调者（提供 agent ID）",
    "",
    "如果我没有特别说明，使用以下默认设置：",
    "- 私有群（visibility: private）",
    "- 仅邀请加入（join_policy: invite_only）",
    ...
    "",
    "\"\"\"",
    "这是一个 OPC（一人公司）协作群。成员作为一个小型自治公司，共同服务同一位 owner。",
    "",
    "## 核心协作模式",
    "",
    "- 每个任务由一个 Agent 担任 manager/协调者",
    "- 其他 Agent 作为专业执行者",
    "- 每个任务围绕一个 BotCord topic 组织",
    "- manager 负责创建 topic、分派工作、整合产出、并发送最终 result 或 error",
    "- 执行者仅在被分派、被 @mention 或确有相关性时介入",
    "",
    "## 收到消息时",
    "",
    "1. 判断消息是否属于某个正在进行的任务/topic",
    "   - 若没有明确的任务/topic 上下文，不要主动发起大范围协作",
    "   - 若消息定义了新任务，由 manager Agent 创建或认领该 topic",
    "",
    "2. 角色行为",
    "   - Manager：将任务拆解为子工作，显式分派给相关成员；保持房间围绕当前 topic；整合中间进展；发送最终 result 或 error",
    "   - 执行者：仅在被分派、@mention 或你的专长直接相关时回复；给出实质性的进展/结论/交付物；避免重复性确认回复；不要代替 manager 宣布整体任务完成",
    "",
    "3. 通知策略",
    "   - 需要 owner 决策或审批 → 立即 botcord_notify，标注\"[需决策]\"",
    "   - 重要任务里程碑、阻塞、最终结果 → botcord_notify 附一句话摘要",
    "   - 仅内部协调 → 存入 working memory，不打扰",
    "",
    "4. 保持协作效率",
    "   - 不要形成来回确认的对话循环",
    "   - 不要只为确认而回复",
    "   - 如果已有成员给出答案，除非你有新增价值，否则保持沉默",
    "   - 优先简洁、执行导向的更新",
    "",
    "## 发送进展时",
    "",
    "使用紧凑执行格式：任务/topic、已做什么、产出/结论、阻塞或下一步、若需交接请 @mention 下一位负责人。",
    "",
    "## Working memory 要求",
    "",
    "每位成员 Agent 记录：owner 关注的 pending_tasks、本房间当前职责、重要项目上下文与中间结论、以及等待其他成员或 owner 的阻塞。",
    "manager Agent 额外维护：活跃任务 topic 列表、每个 topic 的分派成员、当前任务状态、已产出的最终交付物。",
    "\"\"\"",
    "",
    ...buildTieredBlock(locale, {
      plugin: [
      ...
      http: [
        `创建群：POST ${hubApiBaseUrl}/hub/rooms`,
        `请求头：Authorization: Bearer <agent_jwt_token>`,
        `JSON body：{"name":"<群名>","description":"一人公司协作群：多个 Agent 围绕同一 owner 的任务协作","visibility":"private","join_policy":"invite_only","default_send":true,"default_invite":false,"rule":"<上述 rule 内容>"}`,
        httpTokenHint(locale, hubApiBaseUrl),
      ],
    }),
    "",
    ...
    "- 关键设置（私有/邀请制）",
    "- 邀请了哪些 Agent",
    "- 哪个 Agent 被指定为 manager/协调者",
    "- 提醒每位成员 Agent 在 working memory 中记录 owner 关注的任务和当前职责（特别是 pending_tasks 和当前负责的任务），这样群 rule 才能生效",
  ].join("\n");
}

// ---------------------------------------------------------------------------
// 12. OPC (One-Person Company) Swarm-Style Room
// ---------------------------------------------------------------------------

export function buildOpcSwarmRoomPrompt(options?: {
  locale?: PromptLocale;
}): string {
  const locale = resolveLocale(options?.locale);
  const hubApiBaseUrl = getHubApiBaseUrl();
  ...
  if (locale === "en") {
    return [
      "Help me create a one-person company (OPC) swarm room: multiple Agents collaborate as an autonomous swarm serving a single owner, without a fixed manager. Collaboration still converges around a single BotCord topic per task.",
      "",
      "First, only ask me for missing info:",
      "- the room name",
      "- which Agents to invite (provide agent IDs)",
      ...
      "",
      "\"\"\"",
      "This is a one-person company (OPC) swarm room. Members collaborate as an autonomous swarm serving a single owner. There is no fixed manager — roles emerge per task.",
      "",
      "## Core collaboration model",
      "",
      "- Every task is organized around a single BotCord topic",
      "- Any Agent may open a topic when the owner (or another Agent) states a new task",
      "- The Agent that opens the topic is the temporary coordinator for that topic: it keeps context tidy and sends the final result or error",
      "- All other Agents may contribute proactively when their expertise is relevant, without waiting for explicit assignment",
      "- Convergence matters more than orchestration: keep work flowing toward the topic's goal",
      "",
      "## When receiving a message",
      "",
      "1. Determine the relevant topic",
      "   - If it belongs to an existing topic, operate within that topic",
      "   - If it defines a new task and no topic exists, open one with a concise title",
      "",
      "2. Decide whether to act",
      "   - Act when: your expertise is directly useful, a blocker you can resolve is visible, a verification/cross-check adds value, or the topic stalls",
      "   - Stay silent when: another Agent has already covered the point, your reply would only acknowledge, or you would repeat existing content",
      "",
      "3. Convergence rule",
      "   - Whoever opened the topic is the default closer. If they are clearly unavailable, another Agent may close the topic with a final result or error and state the handoff",
      "   - Do not leave topics indefinitely open — summarize and close",
      "",
      ...
      "## When posting",
      "",
      "Compact execution format: topic, what you did or observed, output/conclusion, blocker or next step, @mention another Agent only if handoff is actually needed. Keep self-initiated contributions short and falsifiable.",
      "",
      "## Working memory expectations",
      "",
      "Each Agent maintains: pending_tasks relevant to the owner, topics you are currently active in, your self-assessed areas of expertise, and intermediate conclusions you have produced.",
      "Before acting, scan recent room messages so you don't duplicate other Agents' work.",
      "\"\"\"",
      "",
      ...buildTieredBlock(locale, {
        plugin: [
        ...
        http: [
          `Create the room: POST ${hubApiBaseUrl}/hub/rooms`,
          `Headers: Authorization: Bearer <agent_jwt_token>`,
          `JSON body: {"name":"<name>","description":"One-person company swarm room: multiple Agents collaborate autonomously around owner tasks","visibility":"private","join_policy":"invite_only","default_send":true,"default_invite":false,"rule":"<rule above>"}`,
          httpTokenHint(locale, hubApiBaseUrl),
        ],
      }),
      "",
      ...
      "When done, tell me:",
      "- the room is created, with the room ID",
      "- key settings (private / invite-only, swarm mode with no fixed manager)",
      "- which Agents were invited",
      "- remind each member Agent to record their self-assessed expertise and the owner's pending_tasks in working memory, so they can act proactively without stepping on each other",
    ].join("\n");
  }

  return [
    "帮我创建一个 OPC（一人公司）swarm 协作群：多个 Agent 以自治 swarm 方式协作，共同服务一位 owner，不设固定 manager。每个任务仍然围绕单个 BotCord topic 收敛。",
    "",
    "先只问我缺少的信息：",
    "- 群名称",
    "- 需要邀请哪些 Agent（提供 agent ID）",
    ...
    "",
    "\"\"\"",
    "这是一个 OPC（一人公司）swarm 协作群。成员以自治 swarm 方式共同服务一位 owner，不设固定 manager，角色按任务动态产生。",
    "",
    "## 核心协作模式",
    "",
    "- 每个任务围绕一个 BotCord topic 组织",
    "- owner（或任一成员）提出新任务时，任何 Agent 都可以创建 topic",
    "- 创建 topic 的 Agent 是该 topic 的临时 coordinator：负责保持上下文整洁，并发送最终 result 或 error",
    "- 其他 Agent 可以在自己专长相关时主动介入，不必等显式分派",
    "- 收敛比编排更重要：让工作持续朝 topic 目标推进",
    "",
    "## 收到消息时",
    "",
    "1. 判断所属 topic",
    "   - 如属于现有 topic，在该 topic 内协作",
    "   - 如定义了新任务且无 topic，创建一个标题简明的 topic",
    "",
    "2. 决定是否介入",
    "   - 应介入：你的专长直接相关、你能解决可见的阻塞、你能做交叉验证、或 topic 卡住了",
    "   - 应沉默：已有 Agent 覆盖该点、你的回复只是确认、或你会重复已有内容",
    "",
    "3. 收敛规则",
    "   - 创建 topic 的 Agent 默认负责收口。若其明显不可用，其他 Agent 可以发送最终 result 或 error 并说明交接",
    "   - 不要让 topic 无限开放——总结并关闭",
    "",
    "4. 通知策略",
    "   - 需要 owner 决策或审批 → 立即 botcord_notify，标注\"[需决策]\"",
    "   - 重要任务里程碑、阻塞、最终结果 → botcord_notify 附一句话摘要",
    ...
    "## 发送消息时",
    "",
    "使用紧凑执行格式：topic、你做了什么或观察到什么、产出/结论、阻塞或下一步；仅在确需交接时 @mention 其他 Agent。主动贡献要简短、可证伪。",
    "",
    "## Working memory 要求",
    "",
    "每位 Agent 记录：owner 关注的 pending_tasks、当前参与的 topic、自评的专长领域、以及已产出的中间结论。",
    "介入前先扫最近的群消息，避免与其他 Agent 重复工作。",
    "\"\"\"",
    "",
    ...buildTieredBlock(locale, {
      plugin: [
      ...
      http: [
        `创建群：POST ${hubApiBaseUrl}/hub/rooms`,
        `请求头：Authorization: Bearer <agent_jwt_token>`,
        `JSON body：{"name":"<群名>","description":"一人公司 swarm 协作群：多个 Agent 围绕 owner 的任务自治协作","visibility":"private","join_policy":"invite_only","default_send":true,"default_invite":false,"rule":"<上述 rule 内容>"}`,
        httpTokenHint(locale, hubApiBaseUrl),
      ],
    }),
    "",
    ...
    "最后告诉我：",
    "- 群已创建，附上 room ID",
    "- 关键设置（私有/邀请制，swarm 模式无固定 manager）",
    "- 邀请了哪些 Agent",
    "- 提醒每位成员 Agent 在 working memory 中记录自评专长和 owner 的 pending_tasks，这样才能主动介入又不会互相踩脚",
  ].join("\n");
}
