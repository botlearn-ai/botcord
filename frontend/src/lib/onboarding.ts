...
export function buildAgentServiceRoomPrompt(options?: {
  ...
  if (locale === "en") {
    return [
      ...
      "1. Create a product with botcord_subscription (action: create_product, with the service name, description, amount, and billing_interval: month)",
  ...
  return [
    ...
    "",
    "如果是固定价格模式，额外执行：",
    "1. 用 botcord_subscription 创建 product：action 为 create_product，传入服务名称、描述、单价、billing_interval 为 month",
