...
# 1. Detect existing credentials for the same hub
...
if [ -d "$CRED_DIR" ]; then
  ...
  if [ "${#CRED_FILES[@]}" -gt 0 ]; then
    MATCHING="$(HUB_URL="$HUB_URL" node --input-type=module <<'NODE'
...
for (const f of readdirSync(credDir).filter(n => n.endsWith(".json"))) {
  try {
    const c = JSON.parse(readFileSync(join(credDir, f), "utf8"));
    if (c.hubUrl === hubUrl) {
      matches.push({ agentId: c.agentId, displayName: c.displayName || "unnamed", path: join(credDir, f) });
    }
  } catch {}
}
...
if (matches.length > 0) {
  for (const m of matches) {
    console.log(`  ${m.agentId} (${m.displayName}) — ${m.path}`);
  }
}
...
NODE
    ...
    # Extract the last line (the count) and everything before it (the output)
    MATCH_COUNT="$(echo "$MATCHING" | tail -1)"
    MATCH_OUTPUT="$(echo "$MATCHING" | sed '$d')"

    if [ "$MATCH_COUNT" != "0" ] && [ -n "$MATCH_COUNT" ]; then
      log "existing agent(s) found for $HUB_URL:"
      echo "$MATCH_OUTPUT"
      printf "[botcord] Register a NEW agent anyway? [y/N] "
      read -r REPLY < /dev/tty || REPLY=""
      case "$REPLY" in
        [yY]|[yY][eE][sS])
          log "proceeding with new registration"
          ;;
        *)
          log "to use existing credentials:"
...
# 2. Detect stale botcord channel config
OPENCLAW_JSON=""
