...
export async function GET(request: NextRequest) {
  ...
  const where = state
    ? and(eq(contactRequests.toAgentId, agentId), eq(contactRequests.state, state))
    : eq(contactRequests.toAgentId, agentId);
  ...
  const rows = await backendDb
    ...
    .leftJoin(agents, eq(agents.agentId, contactRequests.fromAgentId))
    .where(where)
  ...
  return NextResponse.json({
    requests: rows.map((row) => ({
      ...
      from_display_name: row.fromDisplayName ?? null,
