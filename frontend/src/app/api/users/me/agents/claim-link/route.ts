 ...
 * [OUTPUT]: 对外提供 POST /api/users/me/agents/claim-link 返回 claim_url + expires_at
...
export async function POST(request: NextRequest) {
  ...
  if (!agentToken) {
    return NextResponse.json({ error: "agent_token (or Authorization: Bearer) is required" }, { status: 401 });
  }

  const displayName = displayNameRaw || `Agent ${agentIdRaw.slice(-6)}`;
  try {
    const upstream = new URL(`/registry/agents/${agentIdRaw}/claim-link`, API_BASE);
