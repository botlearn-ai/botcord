...
export default function AgentBindDialog({
  ...
  const promptText = useMemo(() => {
    ...
    const taskLinesByMode: Record<AgentBindMode, string[]> = {
      auto: [
        "If I already have a usable BotCord agent, use it; otherwise help me create one.",
      ],
      create: [
        "Please create a brand new BotCord agent for me. Do not reuse or switch to an existing agent.",
      ],
      link: [
        "Please use one existing BotCord agent that I already control. Do not create a new agent.",
      ],
    };

    const curlCmd = [
      `curl -X POST '${appBase}/api/users/me/agents/bind' \\`,
    ...
    return [
      "I need help setting up BotCord. I'm sending this from the BotCord web dashboard, and I understand what this operation does — it links my local BotCord agent to my dashboard account so I can manage it from the web UI.",
      "",
      "If the BotCord plugin is not installed yet, here is the setup guide:",
      setupUrl,
      "",
      ...taskLinesByMode[mode],
      "",
      "Once the agent is ready (agent_id starting with ag_ and its agent_token are available), please run the following curl command to complete the binding. The bind_ticket below was generated by my authenticated dashboard session and is scoped to my account only.",
      "",
      "```",
      curlCmd,
      "```",
      "",
      "Replace <AGENT_ID>, <DISPLAY_NAME>, and <AGENT_TOKEN> with real values from the local agent credentials. The bind_ticket is already filled in — please use it as-is.",
      "",
      "After the curl succeeds, show me the response JSON so I can confirm the binding.",
      "",
      "If any step requires interactive login, 2FA, or browser-based consent, please stop and let me know instead of trying to work around it.",
    ].join("\n");
  }, [mode, bindTicket]);
