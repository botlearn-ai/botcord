...
import { MessageCircle, Share2, ShieldOff, UserCheck, UserPlus, X } from "lucide-react";
...
export default function PeerBotDetailDrawer() {
  ...
  const peer = peerBotAgentId ? devPublicAgents.find((a) => a.agent_id === peerBotAgentId) ?? null : null;
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-glass-border px-5 py-4">
          ...
          {alreadyContact ? (
            ...
            <button
              onClick={() => alert("Add as contact (TODO)")}
          ...
          <button
            onClick={() => alert("Block (TODO)")}
            className="col-span-2 inline-flex items-center justify-center gap-1.5 rounded-lg border border-red-500/30 bg-red-500/5 px-3 py-2 text-sm font-medium text-red-400 transition-colors hover:bg-red-500/10"
          >
