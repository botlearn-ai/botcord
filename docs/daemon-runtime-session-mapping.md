...
| Codex | UUIDv7/v4 | `codex exec resume <uuid>`（通过 per-agent CODEX_HOME + AGENTS.md 启用） |
...
- **Resume 已启用**：`thread.started.thread_id` 写入 `state.newSessionId`；有 sessionId 时走 `codex exec resume <sid>`，无则 `codex exec`。
- **systemContext 通过 `<CODEX_HOME>/AGENTS.md` 注入**，不进 transcript：
  - 每个 agent 有独立 `CODEX_HOME` 目录：`~/.botcord/agents/<accountId>/codex-home/`。
  - Adapter 在 spawn 前把 `opts.systemContext` atomic-write 到 `<CODEX_HOME>/AGENTS.md`（tmp + rename）。
  - Adapter 通过 `spawnEnv` 向子进程传 `CODEX_HOME=<path>`。codex 启动时读取该目录的 `AGENTS.md`，作为系统层 instruction，不写入会话 transcript。
- **Sandbox 政策用 `-c` 而非 `-s`**：`codex exec resume` 不接受 `-s/--sandbox/--full-auto`，但 `-c sandbox_mode="..."` 两条路径都支持。Owner 轮：`sandbox_mode="danger-full-access" + approval_policy="never"`；非 owner：`sandbox_mode="workspace-write" + approval_policy="on-request"`。
- **Auth**：`ensureAgentCodexHome()` 在 provision 阶段把 `~/.codex/auth.json` symlink 到 per-agent CODEX_HOME（失败则 copy）。用户 `codex login` 后的 token 通过 symlink 自动可见。
- **Session 隔离**：codex 自己的 `sessions/` 目录也落在 per-agent `CODEX_HOME/sessions/`，不污染用户的 `~/.codex/sessions/` 或其他 agent。
- **没有变**：codex 没有 USD cost 报告，`costUsd` 仍然不填。
...
4. 如果 runtime 没有"不进 transcript 的 system prompt"通道（像早期 codex 那样），优先找文件/环境层注入点（例如自带的 instructions 文件）；再退一步才考虑禁用 resume。
...
Codex 现在也走这套契约（过去是"永远不写"；现在 `thread.started` 会填 `newSessionId`，触发正常 upsert）。
