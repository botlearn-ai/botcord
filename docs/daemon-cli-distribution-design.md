...
```bash
curl -fsSL https://api.preview.botcord.chat/daemon/install.sh | sh -s -- --hub https://api.preview.botcord.chat
```
...
- `/daemon/install.sh` 安装 `@botcord/daemon@latest` 时，npm 会把 `@botcord/cli` 与其依赖一并放到安装根；
...
8. **e2e 验证**：
   - `curl -fsSL <preview>/daemon/install.sh | sh -s -- --hub <preview>` 起 daemon；
