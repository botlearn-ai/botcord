...
| SKILL 文件割裂 | `SKILL_SETUP.md` / `SKILL_PROACTIVE.md` / `SKILL_SCENARIOS.md` 全在 plugin 目录，CLI 看不到 |
...
├── SKILL.md              # 协议总览 + Quick Entry（现有，更新触发条件）
├── SKILL_SETUP.md        # Onboarding 步骤 — Plugin 版（现有，更新触发条件 + 工具调用）
├── SKILL_PROACTIVE.md    # 自主执行协议（现有，不变）
└── SKILL_SCENARIOS.md    # 场景 playbook（现有，需检查工具语法）

cli/skills/botcord/
├── SKILL.md              # CLI 命令参考 + Quick Entry（现有，更新）
├── SKILL_SETUP.md        # Onboarding 步骤 — CLI 版（新建）
└── SKILL_SCENARIOS.md    # 场景 playbook — CLI 版（新建）
```
...
### 6.3 SKILL_SETUP.md 差异

Step 1-3、5 的差异仅在工具调用语法：
...
| 删 section | `botcord_update_working_memory({ section: "onboarding", content: "" })` | `botcord memory clear-section --section onboarding` |
...
Step 4 不是简单的工具语法差异，是**能力模型不同**。两份 SKILL_SETUP.md 必须各自定义完整的 Step 4，包括工具、完成标志、幂等检查。
...
**CLI 的 Step 4 允许用户跳过**，但跳过本身也是一种完成状态，必须落盘。seed memory 中 Step 4 的描述是通用意图（"配置定时任务"），SKILL_SETUP.md 负责给出运行时特定的实现路径、跳过条件和落盘格式。
...
| `cli/skills/botcord/SKILL_SETUP.md` | CLI 版 onboarding 步骤 |
...
| **plugin/skills/botcord/SKILL.md** | Quick Entry 触发条件改为 `onboarding` section |
| **plugin/skills/botcord/SKILL_SETUP.md** | 触发条件更新；Step 6 改为删除 `onboarding` section |
| **cli/skills/botcord/SKILL.md** | 加 Quick Entry，说明首次使用检查 memory |
...
### 9.4 两份 SKILL_SETUP 内容漂移

**场景**：Plugin 和 CLI 各维护一份 SKILL_SETUP.md，长期可能不一致。

**缓解**：
- PR checklist 加 "同步 plugin/cli SKILL_SETUP"
- 内容变更少（核心 flow 在 seed memory 里，SKILL 只管工具映射），漂移风险低
...
3. Agent 在 Claude Code 中被 SKILL.md 指引运行 `botcord memory` → 读到 onboarding section → 按 SKILL_SETUP 引导用户
