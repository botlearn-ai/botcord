...
3. 执行 `botcord-daemon start --hub ... --install-token ...`
...
```ts
function buildStartCommand(token: string): string {
  const app = APP_BASE_URL.replace(/\/$/, "");
  const hub = HUB_BASE_URL;
  return `curl -fsSL ${app}/daemon/install.sh | sh -s -- --hub ${hub} --install-token ${shellQuote(token)}`;
}
```

注意：前端拼 shell 命令时需要 shell quote，不能直接插入 token 或 URL。
...
推荐第一版让 install.sh 显式支持 `--install-token`。`--label` 可以保留为高级可选参数，但 dashboard 默认生成命令不带它。
