...
- Hub 数据模型：`server/hub/models.py`
- Dashboard 聚合接口：`server/hub/routers/dashboard.py`
- Hub 消息流：`server/hub/routers/hub.py`
...
建议在 `server/hub/models.py` 中新增以下模型。
...
建议新增 `server/hub/routers/wallet.py`，并在 `server/hub/main.py` 中挂载。
...
当前 dashboard 概览接口位于 `server/hub/routers/dashboard.py`，建议扩展返回：
...
- `server/hub/models.py`
- `server/hub/enums.py`
- `server/hub/main.py`
- `server/hub/dashboard_schemas.py`
- `server/hub/routers/dashboard.py`
- `server/hub/routers/wallet.py`
- `server/hub/services/wallet.py`
- `server/hub/wallet_schemas.py`
- `server/tests/test_wallet.py`
- `server/tests/test_wallet_dashboard.py`

如果生产环境依赖 SQL migration，则还应新增：

- `server/migrations/007_add_wallet_tables.sql`
