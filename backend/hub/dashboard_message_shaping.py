...
async def load_user_display_names(
    ...
    """Map MessageRecord.source_user_id (Supabase user UUID) -> display_name.

    Both dashboard_user_chat and dashboard_human_room rows store the
    Supabase user UUID in source_user_id, so a single join on
    users.supabase_user_id covers both.
    """
    ...
    try:
        ...
        _logger.warning(
            "load_user_display_names: supabase_user_id lookup failed",
            exc_info=True,
        )
    return out
