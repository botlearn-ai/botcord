...
- `npm run test:integration` runs client integration tests.
- `npx tsc --noEmit` performs a strict TypeScript check before opening a PR.

## Coding Style & Naming Conventions
Use TypeScript with ES modules and strict typing. Match the existing style: 2-space indentation, double quotes, semicolons, and trailing commas in multiline literals and calls. Prefer descriptive factory-style names such as `createRoomsTool`, `createWalletTool`, and `resolveAccountConfig`. Keep new files in `src/` named by concern (`topic-tracker.ts`, `credentials.ts`) and reserve `*.test.ts` / `*.integration.test.ts` for tests.
...
Never commit real `privateKey`, `publicKey`, or credential file contents. Use placeholder IDs like `ag_xxx` and keep live credentials in `~/.botcord/credentials/` or local OpenClaw config, not in repository files.
