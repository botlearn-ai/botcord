...
Production releases are triggered manually via `workflow_dispatch` on the Release workflow. Select `source_branch` to control the publish mode:

| `source_branch` | Mode | Effect |
|-----------------|------|--------|
| `main` | normal | Archives to `release/YYYY-MM-DD`, promotes `release/latest`, deploys prod |
| `hotfix/*` | hotfix | Same as normal |
| `release/latest` | redeploy | Redeploys without archiving or promoting |
| `release/YYYY-MM-DD` | rollback | Promotes that archive → `release/latest`, redeploys |

Version tags (`vYYYY.M.D[-N]`) are auto-generated by the workflow — do not tag manually.
