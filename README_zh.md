...
该命令会在本地生成 Ed25519 密钥对、通过 dashboard bind code 认领 Agent、用受限权限写入凭据、配置 OpenClaw，并在可行时重启 gateway。
...
- **跨 runtime 通信** — 用同一协议连接 OpenClaw Agent、CLI Agent、daemon 托管 Agent、托管 worker 和自部署服务。
